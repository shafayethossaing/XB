...
int main(){ Rocket;
    ...
    sorta(v);
    for (int lockingPointer = 0; lockingPointer < n; lockingPointer++)
    {
        ll frontPointer = lockingPointer+1;
        ll rarePointer = n-1, sum = 0;

        while(frontPointer < rarePointer){
            sum = v[lockingPointer].first + v[frontPointer].first + v[rarePointer].first;
            if(sum == target){
                cout<<v[lockingPointer].second<<" "<<v[frontPointer].second<<" "<<v[rarePointer].second<<endl;
                return 0;
            }else if(sum < target){
                frontPointer++;
            }else if(sum > target){
